...
const quickActions = [
    { label: 'Tambah Pengguna', href: '/users/create', icon: UserPlus, hint: 'Akun baru + role.' },
    { label: 'Kelola Modul', href: '/modules', icon: SquareStack, hint: 'Tree menu & izin.' },
    { label: 'Pengaturan', href: '/settings', icon: SettingsIcon, hint: 'Branding, sistem.' },
];
</script>
...
<template>
    ...
    <AppLayout>
        <div class="space-y-6">
            <PageHeader
                :title="userName ? `Halo, ${userName.split(' ')[0]}` : 'Dashboard'"
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <StatCard label="Total Pengguna" :value="stats?.users ?? 0" :icon="Users" />
                <StatCard label="Total Role" :value="stats?.roles ?? 0" :icon="Shield" />
                <StatCard label="Total Modul" :value="stats?.modules ?? 0" :icon="Layers" />
                <StatCard
                    label="Aktivitas Hari Ini"
                    :value="stats?.logsToday ?? 0"
                    :icon="Activity"
                    hint="terhitung 24 jam terakhir"
                />
            </div>
            ...
            <div class="grid gap-3 lg:grid-cols-3">
                ...
                <Card class="p-2">
                    <div class="px-3 pt-2 pb-1">
                        <p class="text-xs font-semibold text-[var(--text-muted)]">Aksi cepat</p>
                    </div>
